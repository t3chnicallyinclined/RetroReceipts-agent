...
export interface RecentMatch {
	...
	mid?: string;
	match_key?: string;
	session_id?: string; // the ranked set (≤10 games) this game belongs to → opens SessionModal (/session?id=)
...
export interface TeamRecord {
	team: string; // comma-separated char ids, e.g. "44,42,52" (Magneto / Storm / Sentinel)
	games: number;
	wins: number;
}
...
export interface Profile {
	...
	recent?: RecentMatch[];
	teams?: TeamRecord[]; // most-played first (server pre-sorts by games desc)
	vs?: VsRow[]; // full head-to-head list, most-played first
