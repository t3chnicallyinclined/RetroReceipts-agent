<script lang="ts">
	...
	// 🎮 the reserved referee cabinet's Steam join link. An OPEN offer only ever exposes an ARCADE (pool/bot)
	// lobby — the server hides a challenger's own P2P lobby until lock — so `arcade && lobby_id` is the safe
	// gate while open; a LOCKED wager exposes whichever lobby the machine holds. The OS hands the URL to Steam.
	const cabinet = $derived(
		w && w.lobby_id && (w.arcade || st === 'locked')
			? `steam://joinlobby/2634890/${w.lobby_id}/${w.lobby_owner ?? ''}`
			: ''
	);
...
>
	<span class="lab">🪙 Money Match</span>
...
<style>
	...
	/* the JOIN affordance is an <a> (the OS hands the steam:// URL to Steam) — reuse the gold cut, add
	   anchor layout so it centres like the buttons and drops its underline. */
	a.join {
		display: inline-flex;
		align-items: center;
