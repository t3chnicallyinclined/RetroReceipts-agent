...
<nav class="tabbar" aria-label="Primary" style="--tabcols:{NAV.length}">
	{#each NAV as t (t.id)}
...
<style>
	...
	@media (max-width: 720px) {
		.tabbar {
			...
			z-index: 40;
			display: grid;
			/* column count follows NAV length (set inline) so adding a tab never wraps to a 2nd row */
			grid-template-columns: repeat(var(--tabcols, 4), 1fr);
			gap: 2px;
