<script lang="ts">
	...
	// ── presence / idle state (agent-aware; mirrors the desktop's ALWAYS-ON VS panel) ──
	// The web can't read local game memory, so "in a match" comes from the live SSE feed, and the idle states
	// key off the tray agent: online (heartbeated recently) → "looking for opponent"; reported-but-stale/none →
	// "agent offline / no agent". This replaces the old "render nothing unless in a live match" behaviour so the
	// Match tab always shows the YOU vs scanning-opponent card, like the Tauri app.
	const inMatch = $derived(!!(me && mine));
	// "an agent has reported" is the connected signal, matching the top-bar AgentChip (which shows on the same
	// flag). NOTE: the server's last_seen tracks match/registration activity, not the heartbeat, so it can't be
	// trusted for idle liveness — hence we don't assert "offline" here; that awaits a server heartbeat→last_seen fix.
	...
	// character sprite → CharSprite (idle animation → static portrait → abbreviation tile).
...
{#snippet chip(id: number, idx: number, accent: string)}
	<div class="cc" class:point={idx === 0} title={charName(id)}>
		<div class="cface">
			<CharSprite {id} eager={idx === 0} {accent} />
...
{:else if me}
	<section class="mm" class:idle={!inMatch} aria-label="Your current match">
		...
		<!-- YOU (left, orange) — always shown once signed in -->
		<div class="plate p1">
		...
		<div class="center">
			...
			{:else if agentReported}
				<span class="statepill idle"><span class="sdot" aria-hidden="true"></span>LOOKING FOR OPPONENT</span>
			{:else}
				<span class="statepill off">AGENT NOT CONNECTED</span>
			{/if}
		</div>

		<!-- OPPONENT (right, blue) — the live opponent, or a scanning placeholder -->
		<div class="plate p2">
			{#if inMatch}
				<div class="who">
	...
	<!-- picked characters + matchup intel — live-match only (they need a live opponent) -->
	{#if inMatch && (myTeam.length || oppTeam.length)}
		<div class="teams">
			<div class="side me">
				{#each myTeam.slice(0, 3) as id, i (i)}{@render chip(id, i, 'var(--p1)')}{/each}
			</div>
			<div class="tdiv" aria-hidden="true"></div>
			<div class="side opp">
				{#each oppTeam.slice(0, 3) as id, i (i)}{@render chip(id, i, 'var(--p2)')}{/each}
			</div>
		</div>
	{/if}
	...
	{/if}
{:else}
	<!-- signed out — the VS card's empty state -->
	<section class="mm mm-out" aria-label="Your current match">
		<div class="ghostvs" aria-hidden="true">VS</div>
		<div class="out-msg">
...
<style>
	...
	/* ── idle / scanning state (agent-aware; mirrors the desktop "GAME NOT RUNNING / scanning") ──── */
	.statepill {
		display: inline-flex;
		align-items: center;
		gap: 6px;
	...
	.scanbox {
		...
		flex: none;
		border-radius: 12px;
		border: 1px dashed color-mix(in srgb, var(--p2) 45%, var(--line));
		background: color-mix(in srgb, var(--p2) 6%, transparent);
	}
