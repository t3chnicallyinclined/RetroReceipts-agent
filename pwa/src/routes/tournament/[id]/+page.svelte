<script lang="ts">
	...
	// The signed-in viewer organizes this event (owner or co-TO) → gets the Edit link (+ Phase 2/3 controls).
	const isTO = $derived(
...
{:else}
	...
	<section class="hero" style="--acc:{st.cls === 'good' ? 'var(--good)' : st.cls === 'live' ? 'var(--live)' : 'var(--gold)'}">
		...
		<div class="hrow">
			{#if place || doc.online}
				<span class="hi"><span class="flag"><Flag cc={doc.cc} w={16} /></span>{place || 'Online'}</span>
			{/if}
		...
		<div class="hbtm">
			...
			<div class="links">
				...
				{/if}
				{#if isTO}
					<a class="lnk" href="{base}/tournament/create?id={doc.id}">Edit</a>
					<a class="lnk manage" href="{base}/tournament/{doc.id}/manage">⚙ Manage</a>
				{/if}
			</div>
		</div>
	</section>
			...
			{#each regs as r (r.steamid)}
				...
				<div class="pr" class:dropped>
					...
					<a class="pid" href="{base}/u/{r.steamid}">
						...
						<span class="pnm">
							{#if players[r.steamid]?.cc}<span class="pf"><Flag cc={players[r.steamid]?.cc} w={16} /></span>{/if}
							{pname(r.steamid)}
						</span>
					</a>
