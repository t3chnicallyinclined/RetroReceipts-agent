...
{#if auth.authed}
	...
	<!-- Represent — set your country/city so a flag shows by your name and you land on the Regions board -->
	<div class="rail sec-hd">Represent</div>
	<LocationEditor />
{/if}
...
<!-- Feedback -->
<div class="rail sec-hd">Feedback</div>
<div class="card">
	<div class="row">
		<div class="rowlabel">
			<b>Suggest a stat</b>
			<span class="sub">Propose a leaderboard or stat worth tracking — good ideas become boards.</span>
		</div>
		<button class="btn ghost" onclick={() => (showSuggest = true)}>💡 Suggest a stat</button>
	</div>
</div>
...
{/if}
{#if showSuggest}
	<SuggestModal onClose={() => (showSuggest = false)} />
{/if}
